...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($password) {
        ...
        $pdo->prepare("UPDATE users SET email=?, role=?, voucher_balance=?, password=? WHERE id=?")->execute([$email, $role, $voucher, $hash, $id]);
    } else {
        $pdo->prepare("UPDATE users SET email=?, role=?, voucher_balance=? WHERE id=?")->execute([$email, $role, $voucher, $id]);
    }
    header('Location: index.php?msg=User ' . $user['username'] . ' berhasil diupdate');
    exit;
}
?>
<!DOCTYPE html>
...
<head>
<meta charset="UTF-8">
<title>Edit User - VPS Panel</title>
<style>
...
input, select { width: 100%; padding: 10px 14px; background: #0f172a; border: 1px solid #334155; border-radius: 8px; color: #e2e8f0; font-size: 14px; outline: none; }
...
<div class="main">
    <div class="page-header">
        <h1>✏️ Edit User — <?= htmlspecialchars($user['username']) ?></h1>
    </div>
    ...
    <div class="card">
        <form method="POST">
            ...
            <div class="form-group">
                ...
                <div class="hint">Biarkan kosong jika tidak ingin mengubah password</div>
            </div>
            <div class="form-group">
                <label>Email</label>
            ...
            <div class="form-row">
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="voucher" value="<?= $user['voucher_balance'] ?>" min="0">
                </div>
            </div>
